 ...
 * 配置属性类生成器。
 * <p>
 * 通过反射扫描目标类中所有带有 {@link ConfigProperty} 注解的字段，
 * 自动调用 {@link PropertiesUtil#getPropValue} 读取配置文件中对应的属性值并注入，
 * 最终返回一个已填充属性的配置对象实例。
 * </p>
 *
 * <p>使用示例：</p>
 * <pre>
 * TestConfigProperties config = PropertiesGenerator.generate(TestConfigProperties.class);
 * </pre>
 *
 * @author 姚礼林
 * @date 2026/3/25
...
public class PropertiesGenerator {
    ...
    /**
     * 根据目标类上的 {@link ConfigProperty} 注解生成配置属性对象。
     * <p>
     * 遍历目标类的所有字段，对标注了 {@link ConfigProperty} 的字段，通过
     * {@link PropertiesUtil#getPropValue} 读取配置值并注入到对应字段中。
     * 注解中的 {@code cache}、{@code expireSeconds}、{@code required} 会透传给 PropertiesUtil。
     * </p>
     *
     * @param <T>   配置属性类泛型
     ...
     * @throws PropNotConfigureException 如果某个字段标注了 required=true 且配置文件中未配置该属性，则抛出此异常
     */
    public static <T> T generate(Class<T> clazz) throws ConfigGenerateException, PropNotConfigureException {
        try {
            ...
            throw new ConfigGenerateException("生成配置对象失败，类：" + clazz.getName(), e);
        }
    }

    /**
     * 如果字段上有 {@link ConfigProperty} 注解，则读取配置值并注入到该字段。
     *
    ...
    /**
     * 根据 {@link ConfigProperty} 注解的参数调用对应的 PropertiesUtil 重载方法获取配置值。
     * <p>如果注解中的 {@code name} 为空，则使用字段名作为配置项名称。</p>
     *
     * @param annotation 字段上的 ConfigProperty 注解
     * @param fieldName  字段名，当注解 name 为空时作为配置项名称的回退值
     * @return 配置属性值
     * @throws PropNotConfigureException 如果 required=true 且配置值为空时抛出
     */
    private static String fetchValue(ConfigProperty annotation, String fieldName) throws PropNotConfigureException {
        ...
        if (cache) {
            return PropertiesUtil.getPropValue(fileName, name, required, true, expireSeconds);
        }
        return PropertiesUtil.getPropValue(fileName, name, required);
    }
}
